 ...
 * one token at a time (one for words and two for numbers). As in Pascal, these token-processing methods skip leading
 * whitespace, but unlike Pascal, they also skip trailing whitespace on the current input line. Skipping the trailing
 * space allows one to write token-processing programs without worrying about trailing whitespace at the end of a line
 * that might imply that more input is present when it really isn't.
 *
 * <p>
 * TextReader also provides utilities for reading the input one character at a time or one line at a time, in which case
 * whitespace is preserved. As noted above, it also has a utility for peeking one character ahead in the stream. There
 * is also a method for skipping whitespace. A programmer can combine these utilities to define specialized behavior.
 *
 * <p>
 * To simplify differences between operating systems, TextReader skips a "\r" character if it sees it so that the
 ...
 * </blockquote>
 *
 * To construct a reader using a file, the potential I/O error must be caught because, for example, the specified file
 * might not exist. Below is an example of how to construct and manipulate a file reader using a specific file name (in
 * this case, project.dat). If an error occurs, the program exits with a runtime exception.
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * Below is a variation where the user is prompted for a file name and the program loops until a legal file name is
 * given.
 *
 * <blockquote>
 *
 ...
 * </blockquote>
 *
 * The previous two code examples use FileInputStream and IOException, which would require you to include the line below
 ...
 * </blockquote>
 *
 * Questions about this class can be sent to <a href="http://www.cs.arizona.edu/people/reges">Stuart Reges</a>
...
public class TextReader {
    ...
    /**
     * Determine whether or not ends of line are treated as tokens. This method affects only the token-reading methods
     * (readWord, readInt, readDouble).
     *
     * @param flag
