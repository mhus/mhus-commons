...
/**
 * Matches a single value against a condition. The Matcher will compile the condition and speedup if you use the same
 * condition often.
 *
 * <p>
 * Syntax:
 ...
 *
 * <p>
 * Types: - fs - file sysytem like pattern with * - sql - sql like pattern with % - regex (default) - regular expression
 *
 * <p>
 * Operators: - and, &amp;&amp; - And - or, || - Or - Brackets - How brackets work ... - not, ! as negative operator
 *
 * <p>
 * e.g. .*aaa.* .*aaa.* or .*bbb.* .*aaa.* and .*bbb.* .*aaa.* and not .*bbb.* not (.*aaa.* or .*bbb.*) .*xyz.* or
 * (.*aaa.* and .*bbb.*) fs *aaa* sql %aaa%
 *
 * @author mikehummel
 */
    ...
    protected void parse(StringTokenizerParser condition) throws MException {
        ...
        for (String part : condition) {
            ...
            if (condition.isTokenEncapsulated()) {
                ...
                case "fs":
                    if (pattern != null)
                        throw new SyntaxError("type after pattern");
                    pattern = new ModelFs();
                    break;
                case "sql":
                    if (pattern != null)
