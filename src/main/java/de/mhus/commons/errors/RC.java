...
import de.mhus.commons.util.MessageSource;
...
public class RC {
    ...
    public enum STATUS {
        WARNING_TEMPORARILY(199), OK(200), CREATED(201), ACCEPTED(202), WARNING(299), ERROR(400), ACCESS_DENIED(401),
        FORBIDDEN(403), NOT_FOUND(404), CONFLICT(409), GONE(410), TOO_LARGE(413), SYNTAX_ERROR(415), TEAPOT(418),
        USAGE(422), LIMIT(427), INTERNAL_ERROR(500), NOT_SUPPORTED(501), BUSY(503), TIMEOUT(504), TOO_DEEP(508);
    ...
    /**
     * you are not allowed to access the system, causes the client system to ask for a password, exception for
     * authentication
     */
    public static final int ACCESS_DENIED = 401;
    /**
     * you are not allowed to access this resource, user and password is given but access ins not granted, exception for
     * authorization
     */
    ...
    /**
     * conflict state or wrong state, to many retries, key to remove not found, key already set wrong configuration - if
     * not fixed fast, result is null, duplicate entry, device is not enabled ... is not writable / read only
     */
    public static final int CONFLICT = 409; // conflict state or wrong state
    /**
     * indicating that the resource requested by the client has been permanently deleted, and that the client should not
     * expect an alternative redirection or forwarding address
     */
            ...
            for (Object parameter : parameters) {
                ...
                if (parameter != null) {
                    ...
                    if (parameter instanceof Object[])
                        ...
                        addEncoded(sb, parameter, maxSize);
                } else
                    sb.append("null");
            }
    ...
    private static void addEncoded(StringBuilder sb, Object obj, int maxSize) {
        ...
        if (pos < msg.length())
            encodeBackslash(sb, msg.substring(pos), maxSize);
        sb.append("\"");
    }
    ...
    public static Translated translate(Locale locale, String message) {
        ...
        if (message.startsWith("[") && message.endsWith("]")) {
            try {
                ...
                LOGGER.debug("can't parse error message {}", message, t);
            }
        }
    ...
    public static class Translated {
        ...
        public void translate() {
            MessageSource ms = MService.getService(MessageSource.class);
            ...
            for (int i = 1; i < array.size(); i++) {
                ...
                if (item.isArray()) {
                    try {
                        ...
                        LOGGER.debug("can't translate nested message {}", message, t);
                    }
                } else
                    args[i - 1] = item.toString();
            }
            return args;
        }
