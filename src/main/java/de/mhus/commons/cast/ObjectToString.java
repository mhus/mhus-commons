...
public class ObjectToString implements Caster<Object, String> {
    ...
    public String cast(Object in, String def) {
        ...
        if (in instanceof Date)
            return MDate.toIso8601((Date) in);
        if (in instanceof Calendar)
            return MDate.toIso8601((Calendar) in);
        if (in instanceof Throwable)
            return MCast.toString((Throwable) in);
        if (in instanceof byte[])
            return MCast.toString((byte[]) in);
        if (in.getClass().isArray())
            Arrays.asList(in).toString();
