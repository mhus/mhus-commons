 ...
 * <dt>yes
 * <dd>The 10 missing days in 1582 October. (Pope Gregory's correction) 1582 Oct 5 to 1582 Oct 14 never happened.
 * <dt>partly
 * <dd>Britain and its territories (including the USA and Canada) adopted the Gregorian correction in 1752. By then, 11
 * days had to be dropped. 1752 September 3 to 1752 September 13 never happened. However, you can modify constants in
 * BigDate to use the British calendar. Such a change only affects dates prior to 1753 since BigDate calendar is based
 * on Jan 1, 1970. toOrdinal with the Gregorian and British scheme will give the same number for recent dates. You must
 * recompile with the isBritish boolean changed to true.
 * <dt>yes
 * <dd>missing year 0 between 1 BC and 1 AD.
 * <dt>no
 * <dd>in Roman times leap years occurred at irregular intervals, Considered inauspicious, they were avoided during war.
 * Instead we presume leap years every 4 years even back to 999,999BC.
 * <dt>no
 * <dd>leap seconds.
 * </dl>
 *
...
public class BigDate implements Cloneable, Serializable, Comparable<Object> {
     ...
     */
    private static final int[] leap_daysInYearPriorToMonthTable = { 0, 31, 60, /*
                                                                                * J F M
                                                                                */ 91, 121, 152, /* A M J */ 182, 213,
            244, /* J A S */ 274, 305, 335 }; /* O N D */

    /**
     * Constant array: what month does the indexing day number fall in in a leap year? Indexed by ddd Jan 1 = 0.
     */
     ...
     */
    private static final int[] usual_DaysPerMonthTable = { 31, 28, 31, /* J F M */ 30, 31, 30, /* A M J */ 31, 31, 30,
            /* J A S */ 31, 30, 31 }; /* O N D */

    /**
     * Constant array: how many days in the year prior to the first of the given month in a non-leap year. Indexed by
     * Jan=0.
     */
    private static final int[] usual_daysInYearPriorToMonthTable = { 0, 31, 59, /*
                                                                                 * J F M
                                                                                 */ 90, 120, 151, /* A M J */ 181, 212,
            243, /* J A S */ 273, 304, 334 }; /* O N D */

    /**
     * Constant array: what month does the indexing day number fall in in a non-leap year? Indexed by ddd Jan 1 = 0.
     */
    ...
    /**
     * Returns a BigDate object initialised to today's UTC (Greenwich GMT) date, in other words that date the people in
     * Greenwich England think it is right now. It works even if Java's default Timezone is not configured correctly,
     * but it requires your system clock accurately set to UTC time. Experiment setting your system date/time to various
     ...
     * @param asof
     *            usually today, the day you want the age as of. asof must come after birthDate to get a meaningful
     *            result.
     *
     ...
     *         1=Calendar.SUNDAY Not compatiblse with BigDate.getDayOfWeek.
     *
     * @see #isoDayOfWeek
     * @see #dayOfWeek
     ...
     * had built up noticeably. At the suggestion of astronomers Luigi Lilio and Chistopher Clavius, Pope Gregory XIII
     ...
     * @param dayOfWeek
     *            0=Sunday 1=Monday 2=Tuesday 3=Wednesday 4=Thursday 5=Friday 6=Saturday WARNING: not compatible with
     *            1=Calendar.SUNDAY.
     * @param yyyy
     *            year of interest.
     * @param mm
    ...
    public static int toOrdinal(int yyyy, int mm, int dd) {
        ...
        int missingDayAdjust = (yyyy == OJC_lastYYYY && ((mm == OJC_lastMM && dd > OJC_lastDD) || mm > OJC_lastMM))
                ? missingDays : 0;
    ...
    /**
     * Construct a BigDate object given the Propleptic Julian day number. The Propleptic Julian calendar that
     * astronomers use starts with 0 at noon on 4713 BCE January 1. In contrast, BigDate's Ordinal base is 1970 Jan 1.
     *
     * @param prolepticJulianDay
     *            days since 4713 BC Jan 1 noon. Such numbers usually arise in astronomical calculation. You don't need
     *            to concern yourself with the strangeness of the Julian calendar, just its simple day numbering.
     *            BEWARE! after adjusting for noon, fractional parts are discarded. BigDate tracks only dates, not dates
     *            and times. e.g. 2000-3-20 noon is 2,451,624 in proleptic day numbers. 1970-1-1 is 2,440,588 1600-1-1
     *            is 2,305,448 1500-1-1 is 2,268,933 0001-1-1 is 1,721,424 -0001-12-31 is 1,721,423 -0006-1-1 is
    ...
    /**
     * Create a BigDate object from a sting of the form: yyyy-mm-dd must have 4-digit years, and use dashes between the
     ...
     *
     * @param b
     *            an existing BigDate object to use as a model for cloning another.
     */
    ...
    /**
     * Get week number 1 to 53 of the year this date falls in, according to the rules of ISO standard IS-8601 section
     * 5.5. A week that lies partly in one year and partly in another is assigned a number in the year in which most of
     * its days lie. This means that week 1 of any year is the week that contains 4 January, or equivalently week 1 of
     * any year is the week that contains the first Thursday in January. Most years have 52 weeks, but years that start
     * on a Thursday and leap years that start on a Wednesday have 53 weeks. Jan 1 may well be in week 53 of the
     * previous year! Only defined for dates on or after 1600 Jan 01. You can find out how many ISO weeks there are per
     * year with new BigDate( year, 12, 31).getISOWeekNumber();
     *
    ...
    /**
     * @return Julian day number of noon of the date BigDate represents. See notes on the Julian Propleptic calendar
    ...
    public final long getTimeStamp(TimeZone timeZone) {
        ...
        /*
         * Gets the time zone offset, for current date, modified in case of daylight savings. This is the offset to
         * addto UTC to get local time. Places east of Greenwich have a positive offset. :era the era of the given date,
     ...
     * e.g. 1954 September 31 -> 1954 October 1. <br>
     * 1954 October -1 -> 1954 September 29.<br>
     * 1954 13 01 -> 1955 01 01. This lets you do year, month or day arithmetic. normalise does not recompute the
     * ordinal.
     */
     ...
     * @param timeZone
     *            Timezone you want to know the date in at that time. TimeZone.getDefault() TimeZone.getU
     */
    ...
    public String toDowMMDDYY() {
        ...
        return shortDayName[getDayOfWeek()] + " " + MString.toLZ(mm, 2) + "/" + MString.toLZ(dd, 2) + "/"
    ...
    protected final void toGregorian() {
        ...
        if (ordinal > MAX_ORDINAL) {
            ...
            yyyy = Leap400RuleYYYY
                    + flooredMulDiv(ordinal - Jan_01_Leap400RuleYear, 10000, 3652425); /* 365 + 0.25 - 0.01 - 0.0025 */
            /*
