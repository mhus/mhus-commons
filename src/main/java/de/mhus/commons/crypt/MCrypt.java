...
/**
 * This tool is implementing functions to work with encryption and obfuscation to protect data. The algorithm is
 * implemented separate to the usual java.security package. If you want to use the save and proven java implementation
 * do not use this tool. With this tool you do not need any high security patch for your java JRE.
 *
 * <p>
...
public class MCrypt {
    ...
    /*
     * public static AsyncKey createKeyPair(BigInteger prime1, BigInteger prime2) { // (D * E) % z = 1 BigInteger n =
     * prime1.multiply(prime2); BigInteger z = prime1.subtract(BigInteger.ONE).multiply( prime2.subtract(BigInteger.ONE)
    ...
    public static BigInteger[] encodeBytes(AsyncKey key, byte[] in) throws IOException {
        CipherEncodeAsync encoder = new CipherEncodeAsync(key, getRandom());
    ...
    /**
     * Decode the data using Base91 byte encoding and the private key from 'key' using a RSA like algorithm. It's not
     * the java implementation used.
     *
     * @param key
     *            private key
     * @param in
     *            the encoded data presentation
     *
    ...
    /**
     * Decode the data using the private key from 'key' using a RSA like algorithm. It's not the java implementation
     * used.
     *
     * @param key
     *            private key
    ...
    public static OutputStream createCipherOutputStream(OutputStream parent, String passphrase, int version)
        ...
        if (version == 2) {
            ...
            return new SaltOutputStream(new CipherOutputStream(parent, cipher), random,
                    p.length - (random.getInt() % (p.length / 2)), true);
        }
        if (version == 3) {
            // extend passphrase
    ...
    /**
     * Only obfuscate the byte array. The obfuscation is used to confuse the reader but not to secure the data.
     *
     * <p>
    ...
    public static String md5(String real) {
        ...
        try {
            ...
            md.update(real.getBytes(M.UTF_8));
    ...
    /**
     * Encode the byte array synchronous using the pass phrase. The encryption is not stable, two encryptions of the
     * same sample will result in different encryptions.
     *
     * @param passphrase
     * @param in
     *
    ...
    public static byte[] encode(String passphrase, byte[] in) {
        ...
        byte[] pp = passphrase.getBytes(M.UTF_8);
        int ppPos = 0;
        MRandom random = getRandom();
