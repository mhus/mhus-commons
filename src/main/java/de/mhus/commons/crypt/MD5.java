...
public class MD5 {
    ...
    private static final int[] T = new int[] { -680876936, -389564586, 606105819, -1044525330, -176418897, 1200080426,
    ...
    public static byte[] hash(byte[] data) {
        ...
        for (int i = 0; i < sizeBlocks; i++) {
            ...
            for (int j = 0; j < 64; j++, index++)
                buffer[j >>> 2] = ((int) ((index < size) ? data[index] : padding[index - size]) << 24)
                        | (buffer[j >>> 2] >>> 8);
            int orgA = a;
            int orgB = b;
            int orgC = c;
            ...
            for (int j = 0; j < 64; j++) {
                ...
                int tmp = b + Integer.rotateLeft(a + f + buffer[bIndex] + T[j], AMTS[(div << 2) | (j & 3)]);
                a = d;
                d = c;
                c = b;
                b = tmp;
